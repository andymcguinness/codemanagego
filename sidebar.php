<div class="sidebar large-3 small-12 columns">
                <div class="row">
                    <img class="small-4 columns profile-img" src="http://placehold.it/60x60" />
                    <div class="small-8 columns user-info">
                                <div class="row">
                                                <div class="small-12 columns username"><strong><a href="profile.php">Cat McGuinness</a></strong></div>
                                                <div class="small-12 columns inbox"><a href="#"><i class="icon-drawer"></i> Inbox</a></div>
                                </div>
                    </div>
                </div>
                
                <section class="sidebar-content">
                    <ul>
                        <li class="title"><a href="projects"><i class="icon-folder"></i> Projects</a>
                                <ul>
                                                <li><a href="#">Code, Manage, Go!</a></li>
                                                <li><a href="#">Ad Club Site</a></li>
                                                <li><a href="#">Responsive Give Theme</a></li>
                                </ul>
                        </li>
                        <li class="title"><a href="files.php"><i class="icon-library"></i> Repositories</a>
                                <ul>
                                                <li><a href="#">Code, Manage, Go!</a></li>
                                                <li><a href="#">Ad Club Site</a></li>
                                                <li><a href="#">Responsive Give Theme</a></li>
                                </ul>
                        </li>
                    </ul>
                </section>
            </div>